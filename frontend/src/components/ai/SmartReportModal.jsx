...
export function SmartReportModal({
  ...
  const canUseModerationReport = departmentId != null;
  const [reportType, setReportType] = useState(canUseModerationReport ? 'moderation' : 'summary');
  const [status, setStatus] = useState('idle'); // idle | loading | success | error
  const [errorMsg, setErrorMsg] = useState('');

  useEffect(() => {
    if (!canUseModerationReport && reportType === 'moderation') {
      setReportType('summary');
    }
  }, [canUseModerationReport, reportType]);
        ...
        <>
          ...
          >
            ...
            <div className="mb-5 space-y-2">
              {REPORT_TYPES.map((rt) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (rt.id === 'moderation' && !canUseModerationReport) return;
                    setReportType(rt.id);
                  }}
                  disabled={rt.id === 'moderation' && !canUseModerationReport}
                  className={`w-full rounded-xl border p-3 text-left transition-all duration-200 ${reportType === rt.id
                    ? 'border-indigo-500/50 bg-indigo-500/15'
                    : 'border-white/8 bg-white/3 hover:border-white/15'
                    } ${rt.id === 'moderation' && !canUseModerationReport ? 'opacity-60 cursor-not-allowed' : ''}`}
                >
                  ...
                  {rt.id === 'moderation' && !canUseModerationReport && (
                    <p className="mt-1 text-[10px] text-rose-400">Select a department to generate moderation reports.</p>
                  )}
                </button>
              ))}
            </div>
            ...
            <motion.button
              ...
              className={`flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-bold shadow-lg transition-all duration-200 ${status === 'success'
